...
func (alloc *allocator) selectorsMatch(r requestIndices, device *draapi.BasicDevice, deviceMixins []draapi.DeviceMixin, deviceID DeviceID, class *resourceapi.DeviceClass, selectors []resourceapi.DeviceSelector) (bool, error) {
	for i, selector := range selectors {
		...
		var dms []resourceapi.DeviceMixin
		for _, mixin := range deviceMixins {
			...
			if err := draapi.Convert_api_DeviceMixin_To_v1beta1_DeviceMixin(&mixin, &dm, nil); err != nil {
				return false, fmt.Errorf("convert DeviceMixin: %w", err)
			}
			dms = append(dms, dm)
		}
		matches, details, err := expr.DeviceMatches(alloc.ctx, cel.Device{Driver: deviceID.Driver.String(), Attributes: d.Attributes, Capacity: d.Capacity, Mixins: dms})
		if class != nil {
