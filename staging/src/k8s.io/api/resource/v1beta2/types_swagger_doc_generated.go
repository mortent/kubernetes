...
var map_DeviceRequest = map[string]string{
	...
	"firstAvailableOf": "FirstAvailable specified a list of DeviceSubRequests, exactly one of which must be satisfied in order to satisfy this request. The scheduler will attempt to satisfy the list of subrequests in order, so the second will only be attempted if the first entry couldn't be satisfied.",
}
...
var map_DeviceSubRequest = map[string]string{
	"":                "DeviceSubRequest describes a request for device provided in the claim.spec.devices.requests[].firstAvailable array. Each is typically a request for a single resource like a device, but can also ask for several identical devices.\n\nDeviceSubRequest is similar to SpecificDeviceRequest, but doesn't expose the AdminAccess field as that one is only supported when requesting a specific device.\n\nA DeviceClassName is equired. Clients must check that it is indeed set. It's absence indicates that something changed in a way that is not supported by the client yet, in which case it must refuse to handle the request.",
