...
var map_DeviceRequest = map[string]string{
	"":                "DeviceRequest is a request for devices required for a claim. This is typically a request for a single resource like a device, but can also ask for several identical devices. It can also provide a list of alternative subrequests using the firstAvailable list.\n\nA DeviceClassName is currently required if the firstAvailable list is empty. It's absence indicates that something changed in a way that is not supported by the client yet, in which case it must refuse to handle the request. If the firstAvailable list has one or more subrequests, then all other fields except the name must be unset.",
	...
	"firstAvailable":  "FirstAvailable contains subrequests, exactly one of which must be satisfied in order to satisfy this request. This field may only be set in the entries of DeviceClaim.Requests.",
}
...
var map_DeviceRequestAllocationResult = map[string]string{
	...
	"request":     "Request is the name of the request in the claim which caused this device to be allocated. If it references a subrequest in the firstAvailable list on a DeviceRequest, this field must include both the name of the parent request and the subrequest using the format <parent request>/<subrequest>.\n\nMultiple devices may have been allocated per request.",
	"driver":      "Driver specifies the name of the DRA driver whose kubelet plugin should be invoked to process the allocation once the claim is needed on a node.\n\nMust be a DNS subdomain and should end with a DNS domain owned by the vendor of the driver.",
	"pool":        "This name together with the driver name and the device name field identify which device was allocated (`<driver name>/<pool name>/<device name>`).\n\nMust not be longer than 253 characters and may contain one or more DNS sub-domains separated by slashes.",
	"device":      "Device references one device instance via its name in the driver's resource pool. It must be a DNS label.",
