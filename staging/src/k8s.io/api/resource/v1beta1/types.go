...
// claim.spec.devices.requests[].firstAvailableOf slice. Each
...
// or FirstAvailableOf fields, as those can only be set on the top-level
// request.
//
...
type DeviceConstraint struct {
	...
	// the parent request if listed, the constraint will apply to all subrequests.
	//
	// +optional
	// +listType=atomic
...
type DeviceClaimConfiguration struct {
	...
	//
	// References to subrequests must include the name of both the parent request
	// and the subrequest using the format <parent request>/<subrequest>.
	//
	// +optional
	// +listType=atomic
	Requests []string `json:"requests,omitempty" protobuf:"bytes,1,opt,name=requests"`
...
type DeviceRequestAllocationResult struct {
	...
	// firstAvailableOf list on a DeviceRequest, the this field must
