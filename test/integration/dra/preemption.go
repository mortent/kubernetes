...
func testDirectClaimPreemption(tCtx ktesting.TContext) {
	...
	// wait for victim to be marked for eviction (deletion timestamp and disruption target) and finalize it
	finalizePreemptedPod(tCtx, namespace, victim.Name)
...
func testConsumableCapacityPreemption(tCtx ktesting.TContext) {
	...
	// Find and finalize the preempted victim FIRST (which also asserts exactly one is preempted)
	preemptedPodName := finalizeAnyPreemptedPod(tCtx, namespace, []string{victimA.Name, victimB.Name})

	// Preemptor should now be able to schedule
	waitForPodScheduled(tCtx, namespace, preemptor.Name)

	// Verify the correct claim is released
	var preemptedClaimName string
	var otherPodName string
	if preemptedPodName == victimA.Name {
		preemptedClaimName = victimClaimA.Name
		otherPodName = victimB.Name
	} else {
		preemptedClaimName = victimClaimB.Name
		otherPodName = victimA.Name
	}
	waitForClaimReleased(tCtx, namespace, preemptedClaimName)

	// Verify the other victim remains running
	otherPod, err := tCtx.Client().CoreV1().Pods(namespace).Get(tCtx, otherPodName, metav1.GetOptions{})
	tCtx.ExpectNoError(err, "get other victim")
	tCtx.Expect(otherPod.DeletionTimestamp).To(gomega.BeNil(), "Other victim should not be preempted")
}
...
func testPartitionableDevicesPreemption(tCtx ktesting.TContext) {
	...
	// Finalize BOTH victims (which also asserts they are both preempted)
	finalizePreemptedPod(tCtx, namespace, victimA.Name)
	finalizePreemptedPod(tCtx, namespace, victimB.Name)

	// Preemptor should now be able to schedule
	waitForPodScheduled(tCtx, namespace, preemptor.Name)

	// Verify both claims are released
...
func testBindingConditionsPreemption(tCtx ktesting.TContext) {
	...
	victimPod := podWithClaimName.DeepCopy()
	victimPod.Spec.PriorityClassName = lowPriority
	_ = createPod(tCtx, namespace, "-victim", victimPod, victimClaim)
	...
	// Manually deallocate the victim's claim because the scheduler only cancels the binding
	// for PreBind pods and skips deletion, leaving the claim allocated (scheduler bug/limitation).
	manuallyDeallocateClaim(tCtx, namespace, victimClaim.Name)

	// Write binding condition to True for preemptor claim to let it schedule
	err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		latest, err := tCtx.Client().ResourceV1().ResourceClaims(namespace).Get(tCtx, preemptorClaim.Name, metav1.GetOptions{})
		if err != nil {
			return err
		}
		latest.Status.Devices = []resourceapi.AllocatedDeviceStatus{{
			...
			Conditions: []metav1.Condition{{
				...
				Reason:             "Testing",
				Message:            "The test is letting the preemptor schedule.",
			}},
		}}
		_, err = tCtx.Client().ResourceV1().ResourceClaims(namespace).UpdateStatus(tCtx, latest, metav1.UpdateOptions{})
		return err
	})
	tCtx.ExpectNoError(err, "add binding condition to preemptor claim")
...
func testPriorityAwarePreemption(tCtx ktesting.TContext) {
	...
	// Finalize Victim A (which is the one that should be preempted)
	finalizePreemptedPod(tCtx, namespace, victimA.Name)

	// Preemptor should now be able to schedule
	waitForPodScheduled(tCtx, namespace, preemptor.Name)

	// Verify that Victim B remains running
...
// finalizePreemptedPod waits for a pod to be marked for preemption and then force-deletes it.
func finalizePreemptedPod(tCtx ktesting.TContext, namespace, podName string) {
	tCtx.Helper()
	waitForPodPreempted(tCtx, namespace, podName)
	err := tCtx.Client().CoreV1().Pods(namespace).Delete(tCtx, podName, metav1.DeleteOptions{GracePeriodSeconds: ptr.To(int64(0))})
	tCtx.ExpectNoError(err, fmt.Sprintf("force delete preempted pod %s", podName))
}

// finalizeAnyPreemptedPod waits for one of the candidate pods to be marked for preemption,
// force-deletes it, and returns its name. Used when we don't know which specific pod will be preempted.
func finalizeAnyPreemptedPod(tCtx ktesting.TContext, namespace string, podNames []string) string {
	tCtx.Helper()
	var preemptedPodName string
	tCtx.Eventually(func(tCtx ktesting.TContext) bool {
		for _, name := range podNames {
			p, err := tCtx.Client().CoreV1().Pods(namespace).Get(tCtx, name, metav1.GetOptions{})
			if err != nil {
				continue
			}
			if p.DeletionTimestamp != nil {
				...
				if cond != nil && cond.Status == v1.ConditionTrue {
					preemptedPodName = name
					return true
				}
			}
		}
		return false
	}).WithTimeout(30*time.Second).Should(gomega.BeTrue(), "One of the pods should be preempted")

	err := tCtx.Client().CoreV1().Pods(namespace).Delete(tCtx, preemptedPodName, metav1.DeleteOptions{GracePeriodSeconds: ptr.To(int64(0))})
	tCtx.ExpectNoError(err, fmt.Sprintf("force delete preempted pod %s", preemptedPodName))
	return preemptedPodName
}

// manuallyDeallocateClaim waits for the claim to be unreserved and then clears its status allocation
// to work around scheduler limitations where it skips deletion for PreBind pods.
func manuallyDeallocateClaim(tCtx ktesting.TContext, namespace, claimName string) {
	tCtx.Helper()
	waitForClaimReleased(tCtx, namespace, claimName)
	tCtx.Eventually(func(tCtx ktesting.TContext) error {
		...
		_, err = tCtx.Client().ResourceV1().ResourceClaims(namespace).UpdateStatus(tCtx, c, metav1.UpdateOptions{})
		return err
	}).WithTimeout(30*time.Second).Should(gomega.Succeed(), fmt.Sprintf("manually deallocate claim %s", claimName))
}

// testLeakedClaimPreemption runs preemption tests where the ResourceClaim controller
// is intentionally disabled to simulate claim leakage.
func testLeakedClaimPreemption(tCtx ktesting.TContext) {
	tCtx.Parallel()

	startScheduler(tCtx)
	// We intentionally do NOT call startClaimController(tCtx) here.

	runSubTest(tCtx, "cascading-preemption-leaked-claims", testCascadingPreemptionLeakedClaims)
}

// testCascadingPreemptionLeakedClaims verifies that a high-priority pod will trigger
// cascading preemption of all low-priority pods on the node if their resource claims
// fail to deallocate.
func testCascadingPreemptionLeakedClaims(tCtx ktesting.TContext) {
	namespace := createTestNamespace(tCtx, nil)
	class, driverName := createTestClass(tCtx, namespace)

	// Create two devices on worker-0
	slice := st.MakeResourceSlice("worker-0", driverName).Obj()
	slice.Spec.Devices = []resourceapi.Device{
		{
			Name: "device-1",
	...
	// Create and schedule Victim 1
	victimClaim1 := createClaim(tCtx, namespace, "-victim-1", class, claim)
	victimPod1 := podWithClaimName.DeepCopy()
	victimPod1.Spec.PriorityClassName = lowPriority
	victim1 := createPod(tCtx, namespace, "-victim-1", victimPod1, victimClaim1)
	victim1 = waitForPodScheduled(tCtx, namespace, victim1.Name)

	// Create and schedule Victim 2
	victimClaim2 := createClaim(tCtx, namespace, "-victim-2", class, claim)
	victimPod2 := podWithClaimName.DeepCopy()
	victimPod2.Spec.PriorityClassName = lowPriority
	victim2 := createPod(tCtx, namespace, "-victim-2", victimPod2, victimClaim2)
	victim2 = waitForPodScheduled(tCtx, namespace, victim2.Name)

	// Create Preemptor requesting a device
	preemptorClaim := createClaim(tCtx, namespace, "-preemptor", class, claim)
	preemptorPod := podWithClaimName.DeepCopy()
	preemptorPod.Spec.PriorityClassName = highPriority
	preemptor := createPod(tCtx, namespace, "-preemptor", preemptorPod, preemptorClaim)

	// 1. The scheduler selects and marks Victim 1 for eviction. We force-delete it.
	finalizePreemptedPod(tCtx, namespace, victim1.Name)

	// Trigger a retry of the preemptor by updating the DeviceClass.
	// This is necessary because the Pod delete event alone does not trigger rescheduling
	// for pods that failed scheduling due to DynamicResources.
	classObj, err := tCtx.Client().ResourceV1().DeviceClasses().Get(tCtx, class.Name, metav1.GetOptions{})
	tCtx.ExpectNoError(err)
	if classObj.Annotations == nil {
		classObj.Annotations = make(map[string]string)
	}
	classObj.Annotations["dummy-update"] = "1"
	_, err = tCtx.Client().ResourceV1().DeviceClasses().Update(tCtx, classObj, metav1.UpdateOptions{})
	tCtx.ExpectNoError(err)

	// 2. Because the claim controller is not running, the claim for victim-1 is NOT deallocated.
	// The scheduler retries, fails filtering, and enters preemption again, selecting Victim 2.
	finalizePreemptedPod(tCtx, namespace, victim2.Name)

	// 3. Since both victim claims are now leaked, the preemptor remains unschedulable.
	waitForPodUnschedulable(tCtx, namespace, preemptor.Name)
}
