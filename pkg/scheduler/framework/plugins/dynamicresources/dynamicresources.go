	...
	for _, resource := range pod.Spec.ResourceClaims {
		...
		if pl.draManager != nil {
			...
			if simulatedUsersCount >= actualUsersCount {
				...
				for _, consumer := range claim.Status.ReservedFor {
					if consumer.Resource != "pods" || consumer.APIGroup != "" {
						canRelease = false
						logger.V(6).Info("Cannot release claim during preemption simulation: reserved for non-pod resource", "claim", name, "consumer", consumer)
						break
					}
					if !state.simulatedRemovedUsersForClaim[name].Has(consumer.UID) {
						canRelease = false
						logger.V(6).Info("Cannot release claim during preemption simulation: consumer not in simulated removed users", "claim", name, "consumerUID", consumer.UID)
						break
					}
				}
			}
			...
			if canRelease {
				...
				logger.V(6).Info("Simulated released claim during preemption", "claim", name)
			}
