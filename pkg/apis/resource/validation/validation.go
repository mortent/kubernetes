...
func validateDeviceRequest(request resource.DeviceRequest, fldPath *field.Path, stored bool) field.ErrorList {
	...
	case 1:
	default:
		allErrs = append(allErrs, field.Invalid(fldPath, nil, "exactly one of `exactly` or `firstAvailable` is required"))
	}
	return allErrs
}
...
func validateResourceSliceSpec(spec, oldSpec *resource.ResourceSliceSpec, fldPath *field.Path) field.ErrorList {
	...
	allErrs = append(allErrs, validateDevicesNoCycles(spec.Devices, fldPath.Child("devices"))...)

	return allErrs
}

// validateDevicesNoCycles checks if the consumesCapacityFrom references in composite
// devices form reference cycles. This is not supported so it will reported as errors.
func validateDevicesNoCycles(devices []resource.Device, fldPath *field.Path) field.ErrorList {
	var allErrs field.ErrorList
	deviceToSourceMapping := make(map[string]string)
	for _, device := range devices {
		if device.Composite == nil {
			continue
		}
		// If a device has more than one entry in the ConsumesCapacityFrom
		// field, it is not valid. So we skip this check.
		if len(device.Composite.ConsumesCapacityFrom) > 1 {
			return allErrs
		}
		if len(device.Composite.ConsumesCapacityFrom) == 1 {
			deviceToSourceMapping[device.Name] = device.Composite.ConsumesCapacityFrom[0].Name
		} else {
			deviceToSourceMapping[device.Name] = ""
		}
	}
	...
	for _, device := range devices {
		...
		if seenDevices.Has(device.Name) {
			continue
		}
		deviceName := device.Name
		path := []string{deviceName}
		for {
			var found bool
			deviceName, found = deviceToSourceMapping[deviceName]
			// If no device is found, then there are a reference to a device
			// that doesn't exist in the ResourceSlice. Ignore that here since
			// we have separate validation for that.
			if !found {
				break
			}
			// If the deviceName is an empty string, we have found the
			// source device (i.e. the root of the tree of devices) without
			// detecting a cycle.
			if deviceName == "" {
				break
			}
			// If we encounter an already visited device during traversal,
			// we can stop. If the device is part of a cycle, it will already
			// have been detected.
			if seenDevices.Has(deviceName) {
				break
			}
			seenDevices.Insert(deviceName)
			// If we encounter the same device more than once during traversal,
			// we have found a cycle.
			if index := slices.Index(path, deviceName); index != -1 {
				path = append(path, deviceName)
				cycle := strings.Join(path[index:], " -> ")
				allErrs = append(allErrs, field.Invalid(fldPath, "", fmt.Sprintf("`consumesCapacityFrom` references can not form cycle. Found cycle: %s", cycle)))
				break
			}
			path = append(path, deviceName)
		}
