...
func TestValidateResourceSlice(t *testing.T) {
	...
	scenarios := map[string]struct {
		...
		"composite-device-with-too-many-device-refs": {
			wantFailures: field.ErrorList{field.TooMany(field.NewPath("spec", "devices").Index(0).Child("composite", "consumesCapacityFrom"), 2, 1)},
			slice: func() *resourceapi.ResourceSlice {
				slice := testResourceSliceWithCompositeMixin(goodName, goodName, goodName, 1, 3)
		...
		"composite-device-with-circular-device-refs": {
			wantFailures: field.ErrorList{
				field.Invalid(field.NewPath("spec", "devices"), "", "`consumesCapacityFrom` references can not form cycle. Found cycle: device-0-0 -> device-0-1 -> device-0-2 -> device-0-0"),
			},
			slice: func() *resourceapi.ResourceSlice {
				slice := testResourceSliceWithCompositeMixin(goodName, goodName, goodName, 1, 3)
				slice.Spec.Devices[0].Composite.ConsumesCapacityFrom = []resourceapi.DeviceRef{
				...
				slice.Spec.Devices[2].Composite.ConsumesCapacityFrom = []resourceapi.DeviceRef{
					{
						Name: "device-0-0",
					},
				}
		...
		"composite-devices-with-multiple-circular-device-refs": {
			wantFailures: field.ErrorList{
				field.Invalid(field.NewPath("spec", "devices"), "", "`consumesCapacityFrom` references can not form cycle. Found cycle: device-0-0 -> device-0-1 -> device-0-2 -> device-0-0"),
				field.Invalid(field.NewPath("spec", "devices"), "", "`consumesCapacityFrom` references can not form cycle. Found cycle: device-1-0 -> device-1-1 -> device-1-2 -> device-1-0"),
			},
			slice: func() *resourceapi.ResourceSlice {
				slice := testResourceSliceWithCompositeMixin(goodName, goodName, goodName, 2, 3)
				slice.Spec.Devices[0].Composite.ConsumesCapacityFrom = []resourceapi.DeviceRef{
					{
						Name: "device-0-1",
					},
				...
				slice.Spec.Devices[3].Composite.ConsumesCapacityFrom = []resourceapi.DeviceRef{
					{
						Name: "device-1-1",
					},
				}
				slice.Spec.Devices[4].Composite.ConsumesCapacityFrom = []resourceapi.DeviceRef{
					{
						Name: "device-1-2",
					},
				}
				slice.Spec.Devices[5].Composite.ConsumesCapacityFrom = []resourceapi.DeviceRef{
					{
						Name: "device-1-0",
					},
				}
