...
func TestValidateResourceSlice(t *testing.T) {
	...
	scenarios := map[string]struct {
		...
		"max-number-of-devices-with-consumes-counters": {
			slice: func() *resourceapi.ResourceSlice {
				slice := testResourceSlice(goodName, goodName, goodName, resourceapi.ResourceSliceMaxDevicesWithConsumesCounters)
				for i := range slice.Spec.Devices {
		...
		"too-many-devices-with-consumes-counters": {
			wantFailures: field.ErrorList{field.TooMany(field.NewPath("spec", "devices"), resourceapi.ResourceSliceMaxDevicesWithConsumesCounters+1, resourceapi.ResourceSliceMaxDevicesWithConsumesCounters)},
			slice: func() *resourceapi.ResourceSlice {
				slice := testResourceSlice(goodName, goodName, goodName, resourceapi.ResourceSliceMaxDevicesWithConsumesCounters+1)
				slice.Spec.Devices[0].ConsumesCounters = []resourceapi.DeviceCounterConsumption{
					{
						CounterSet: "counterset-0",
						Counters:   testCounters(),
