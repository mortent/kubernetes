...
func TestValidateClaimTemplate(t *testing.T) {
	...
	scenarios := map[string]struct {
		...
		"prioritized-list": {
			wantFailures: nil,
			template:     testClaimTemplate(goodName, goodNS, validClaimSpecWithFirstAvailableOf),
		},
		"proritized-list-class-name-on-parent": {
			wantFailures: field.ErrorList{field.Invalid(field.NewPath("spec", "spec", "devices", "requests").Index(0).Child("deviceClassName"), goodName, "must not be specified when firstAvailableOf is set")},
			template: func() *resource.ResourceClaimTemplate {
				template := testClaimTemplate(goodName, goodNS, validClaimSpecWithFirstAvailableOf)
				template.Spec.Spec.Devices.Requests[0].Exactly.DeviceClassName = goodName
				return template
			}(),
		},
		"prioritized-list-bad-class-name-on-subrequest": {
			wantFailures: field.ErrorList{field.Invalid(field.NewPath("spec", "spec", "devices", "requests").Index(0).Child("firstAvailableOf").Index(0).Child("deviceClassName"), badName, "a lowercase RFC 1123 subdomain must consist of lower case alphanumeric characters, '-' or '.', and must start and end with an alphanumeric character (e.g. 'example.com', regex used for validation is '[a-z0-9]([-a-z0-9]*[a-z0-9])?(\\.[a-z0-9]([-a-z0-9]*[a-z0-9])?)*')")},
			template: func() *resource.ResourceClaimTemplate {
				template := testClaimTemplate(goodName, goodNS, validClaimSpecWithFirstAvailableOf)
...
func TestValidateClaimTemplateUpdate(t *testing.T) {
	...
	scenarios := map[string]struct {
		...
		"prioritized-listinvalid-update-class": {
			...
			oldClaimTemplate: testClaimTemplate(goodName, goodNS, validClaimSpecWithFirstAvailableOf),
			update: func(template *resource.ResourceClaimTemplate) *resource.ResourceClaimTemplate {
				template.Spec.Spec.Devices.Requests[0].FirstAvailable[0].DeviceClassName += "2"
				return template
			},
