	...
	for _, apiVersion := range apiVersions {
		t.Run(apiVersion, func(t *testing.T) {
			...
			testCases := map[string]struct {
				...
				// spec.sharedCounters.counters
				"valid: provided counters for counter set": {
					input: mkResourceSliceWithSharedCounters(tweakSharedCounter("valid-key")),
				},
				"invalid: missing counters for counter set": {
					input: mkResourceSliceWithSharedCounters(tweakSharedCounter()),
					expectedErrs: field.ErrorList{
						field.Required(field.NewPath("spec").Child("sharedCounters").Index(0).Child("counters"), ""),
					},
				},
				// spec.devices.consumesCounters.counters
				"valid: provided counters for device counter consumption": {
					input: mkResourceSliceWithDevices(tweakDeviceCounter("valid-key")),
				},
				"invalid: missing counters for device counter consumption": {
					input: mkResourceSliceWithDevices(tweakDeviceCounter()),
					expectedErrs: field.ErrorList{
...
func tweakSharedCounter(keys ...string) func(*resource.ResourceSlice) {
	return func(rs *resource.ResourceSlice) {
		if len(keys) == 0 {
			rs.Spec.SharedCounters[0].Counters = nil
			return
		}
		counters := make(map[string]resource.Counter, len(keys))
		for _, key := range keys {
			counters[key] = resource.Counter{}
		}
...
func tweakDeviceCounter(keys ...string) func(*resource.ResourceSlice) {
	return func(rs *resource.ResourceSlice) {
		if len(keys) == 0 {
